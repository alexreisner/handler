...
Transliteration (using reasonable ASCII approximations non-ASCII characters) is attempted, using the excellent unidecode gem (http://rubyforge.org/projects/unidecode) if it's available.  


== To-do List

* optionally ensure that handle is unique (auto-append number)
